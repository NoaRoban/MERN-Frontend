...
import { StatusBar, StyleSheet, Text, View, Image, TouchableOpacity, Button, Alert, TextInput, FlatList, TouchableHighlight, SafeAreaView, ScrollView } from 'react-native';
...
const MyPostList: FC<{ route: any, navigation: any }> = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const unsubscribe = navigation.addListener('focus', async () => {
            let posts: Post[] = []
            try {
                posts = await UserModel.getAllMyPosts(userInfo.id)
                console.log("fetching posts user complete")
            } catch (err) {
                console.log("fail fetching posts user " + err)
            }
